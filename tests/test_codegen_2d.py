...
def test_linear_form_11():
    # ... parse the Vale code
    stmts  = "Domain(dim=2,kind='structured') :: Omega" + "\n"
    stmts += "Space(domain=Omega,kind='h1')   :: V"     + "\n"
    stmts += "Function(x,y)                   :: f"     + "\n"
...
def test_linear_form_12():
    ...
    stmts += "Space(domain=Omega,kind='h1')   :: V"     + "\n"
    stmts += "Function(x,y)                   :: f"     + "\n"
    stmts += "Function(x,y)                   :: g"     + "\n"
    stmts += "b(v::V) := < f * dx(v) + g * dy(v) >_Omega"           + "\n"
...
def test_linear_form_13():
    ...
    stmts += "Function(x,y)                   :: f"     + "\n"
    stmts += "Function(x,y)                   :: g"     + "\n"
    stmts += "Function(x,y)                   :: h"     + "\n"
    stmts += "b(v::V) := < f * dxx(v) + g * dxy(v) + h * dyy(v) >_Omega" + "\n"
...
def test_bilinear_form_11():
    ...
    stmts  = "Domain(dim=2,kind='structured') :: Omega" + "\n"
    stmts += "Space(domain=Omega,kind='h1')   :: V"     + "\n"
    stmts += "a(v::V, u::V) := < dx(v) * dx(u) + dy(v) * dy(u) >_Omega"
...
def test_bilinear_form_21():
    ...
    stmts  = "Domain(dim=2,kind='structured') :: Omega" + "\n"
    stmts += "Space(domain=Omega,kind='h1')   :: V"     + "\n"
    stmts += "a1(v::V, u::V) := < dx(v) * dx(u) + dy(v) * dy(u) >_Omega" + "\n"
    stmts += "a2(v::V, u::V) := < v * dy(u) >_Omega"         + "\n"
